        ...
        db.query(query, [username], (err, res) => {
			...
			if (res.length > 0){
				...
				generate_token().then(token => {
					bcrypt.hash("asdasd", SALT_ROUNDS, (err, hash) => {
						let entity = {
							username: username,
							password: hash,
							level: -1,
...
function accountVerify(username, email, token){
	return new Promise((resolve, reject) => {
		let query = "SELECT * FROM account WHERE username = ? email = ? token ?"
		db.query(query, (err, res) => {
			...
			if (res.length > 0){
				...
				dbbase.updatetb('account', 'level', entity);
				resolve('Verify successfully');
			}
			else{
